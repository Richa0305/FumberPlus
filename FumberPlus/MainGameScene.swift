...
class MainGameScene: SKScene,GADBannerViewDelegate {
    ...
    func addCircle(){
        ...
        if highscore <= 60 {
            countdown(circle: circle, steps: 10, duration: 10) {
                self.GameOver()
            }
    ...
    func addNewNumberBlock(superNode:SKShapeNode) -> SKShapeNode{
        ...
        mainNode.fillColor = SKColor(hex: 0x008000)
        ...
        if highscore >= 30 && highscore < 60{
            ...
            randomNumberForlabel = arc4random()%40 + 1
        }else if  highscore >= 700 && highscore < 1000{
            randomNumberForlabel = arc4random()%50 + 1
